...
```{r}
root_growth_inh_0 <- read_xlsx("elife-54740-fig1-figsupp3-data1-v2_data-only_tir1afb1.xlsx", sheet = "Primary Root Growth Inh EtOH", skip = 1)
root_growth_inh_20 <- read_xlsx("elife-54740-fig1-figsupp3-data1-v2_data-only_tir1afb1.xlsx", sheet = "Primary Root Growth Inh 20 nM", skip = 1)
root_growth_inh_100 <- read_xlsx("elife-54740-fig1-figsupp3-data1-v2_data-only_tir1afb1.xlsx", sheet = "Primary Root Growth Inh 100 nM", skip = 1)
root_growth_inh_500 <- read_xlsx("elife-54740-fig1-figsupp3-data1-v2_data-only_tir1afb1.xlsx", sheet = "Primary Root Growth Inh 500 nM", skip = 1)
...
```{r}
leveneTest(percent_inh ~ genotype, filter(root_growth_inh, treatment == 100))
leveneTest(percent_inh ~ TIR1*AFB1, filter(root_growth_inh, treatment == 100))
leveneTest(percent_inh ~ genotype, filter(root_growth_inh, treatment == 0))
leveneTest(percent_inh ~ TIR1*AFB1, filter(root_growth_inh, treatment == 0))
qqnorm(filter(root_growth_inh, treatment == 100)$percent_inh)
qqline(filter(root_growth_inh, treatment == 100)$percent_inh)
qqnorm(filter(root_growth_inh, treatment == 0)$percent_inh)
qqline(filter(root_growth_inh, treatment == 0)$percent_inh)
skewness(filter(root_growth_inh, treatment == 100)$percent_inh)
skewness(filter(root_growth_inh, treatment == 0)$percent_inh)
```

The 100 nM data do violate homogeneity of variance, but it seems from reading of https://stats.stackexchange.com/questions/56971/alternative-to-one-way-anova-unequal-variance, that Levene's test is more sensitive than ANOVA, and our ANOVA will lose power if this assumption is invalid, so we would be less likely to see significant effects. However, there are far fewer samples for *afb1* and *tir1afb1*. To minimize the effects of this heterogeneity we could include only *tir1* and Col-0 from the same batches as *afb1* and *tir1afb1*.
